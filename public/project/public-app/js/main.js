...
function switchTab(t) {
    app.tab = t;
    const tabTemario = document.getElementById('tab-temario');
    const tabFav = document.getElementById('tab-favoritos');
    if (tabTemario && tabFav) {
        tabTemario.classList.toggle('active', t === 'temario');
        tabFav.classList.toggle('active', t === 'favoritos');
    }
    renderSidebar();
...
function toggleSidebar() {
    ...
    if (isOpen && app.tab === 'favoritos') {
        ...
        if (tabTemario && tabFav) {
            tabTemario.classList.add('active');
...
function attemptStartQuiz() {
    if (currentUser && Storage.hasAccess()) { startQuiz('premium'); } else { if (app.quizUsed[app.currentIdx]) { openSales(); } else { startQuiz('free_trial'); } }
}
...
async function fetchData() {
    ...
    loadLesson(Math.min(app.currentIdx, app.flatList.length - 1), false);
